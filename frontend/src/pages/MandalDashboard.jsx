...
const MandalDashboard = () => {
        ...
        const fetchData = async () => {
            try {
                ...
                setStats({
                    ...
                    villages: statsRes.data.villages || []
                });
    ...
    // 3. Caste Distribution (Simple Pie/Bar)
    const casteData = analytics?.demographics?.caste?.map((c, index) => ({
        name: c._id || 'Unknown',
        value: c.count,
        color: ['#F59E0B', '#8B5CF6', '#10B981', '#3B82F6', '#EF4444'][index % 5]
    })) || [];

    // --- MAP HELPERS ---
    const getCoordinates = (inputString, seed = '') => {
        // Simple hash-based coordinate generator for demo purposes (same as before)
        ...
        // Base around Nalgonda roughly
        const latBase = ((hash % 1000) - 500) / 30000;
        const lngBase = (((hash >> 5) % 1000) - 500) / 30000;
        return [17.0500 + latBase, 79.2667 + lngBase];
    };

    const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, textAnchor, name, value }) => {
        // For Pie Charts
        return null; // Using Tooltip instead for cleaner look
    };
                        ...
                        <div className="px-4 md:px-8 -mt-10 pb-12 w-full space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in slide-in-from-bottom-4 duration-500">
                                <StatCard
                                    ...
                                    subtext={`Across ${stats.villages.length} Villages`}
                                    icon={FaUsers}
                                ...
                                <StatCard
                                    ...
                                    value={stats.villages.filter(v => v.status === 'Active').length.toLocaleString()}
                                    subtext={`Out of ${stats.villages.length} Total`}
                                    icon={FaMapMarkedAlt}
                                    ...
                                    <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
                                        <div className="flex items-center justify-between mb-6">
                                            <h3 className="font-bold text-[#1e2a4a] text-lg flex items-center gap-2">
                                                <FaChartBar className="text-[#f59e0b]" /> Top Villages by Membership
                                            </h3>
                                        </div>
                            ...
                            <div className="space-y-4" id="village-breakdown">
                                ...
                                {loading ? (
                                    ...
                                    <div className="p-12 text-center text-slate-400 bg-white rounded-xl border border-dashed border-slate-300">
                                        No villages assigned to this area.
                                    </div>
                                ) : (
                                    <>
                                        {/* CARDS VIEW */}
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                                {stats.villages.map((village, idx) => (
                                                    <VillagePerformanceCard
                                                        ...
                                                        pending={village.pending} // Pass pending count
                                                    />
                                                ))}
                                            </div>
                                        )}
