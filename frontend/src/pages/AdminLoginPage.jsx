...
const AdminLoginPage = () => {
    ...
    const handleSendOtp = async (nextMode) => {
        ...
        setLoading(true);
        try {
            // Note: Backend handles userType lookup if needed, but for 'request-otp' likely needs hint
            // Standard authController.js uses 'userType' to find collection
            await API.post('/auth/request-otp', { mobile, userType: 'ADMIN' });
    ...
    const handleVerifyOtp = async (nextMode) => {
        ...
        setLoading(true);
        try {
            await API.post('/auth/verify-otp', { mobile, otp, userType: 'ADMIN' });
            setViewMode(nextMode);
            setFeedback(null);
        } catch (error) {
    ...
    const handleCreateMpinSubmit = async () => {
        if (mpin.length !== 4 && mpin.length !== 6) {
            setFeedback({ type: 'error', text: 'MPIN must be 4 or 6 digits' });
            return;
        }
        ...
        try {
            await API.post('/auth/create-mpin', {
                mpin,
                deviceId: getDeviceId() // Note: Backend handles creating MPIN for the currently authenticated user session (from verify-otp token)
            });

            setFeedback({ type: 'success', text: 'MPIN Created Successfully! Logging in...' });

            setTimeout(() => {
                handleMpinLogin(); // Use state MPIN, don't pass mobile!
            }, 1000);
    ...
    const handleResetMpinSubmit = async () => {
        ...
        try {
            await API.post('/auth/reset-mpin', {
                newMpin: mpin,
                deviceId: getDeviceId(),
                userType: 'ADMIN' // Still passed to handle any backend fallback, but JWT is priority
            });
    ...
    const LogoHeader = () => (
        <div className="mb-8 flex flex-col items-center text-center">
            ...
            <p className="text-[#1e2a4a] text-xs font-bold uppercase tracking-wider mt-1">Admin Login</p>
        </div>
    );
                ...
                {viewMode === 'DEFAULT' && (
                    <div className="animate-fade-in flex flex-col items-center">
                        ...
                        <div className="w-full mb-6">
                            <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2 text-center">Enter Admin MPIN</label>
                            <MpinInput
                                value={mpin}
                                onChange={setMpin}
                                length={4}
                    ...
                    <div className="animate-fade-in">
                        ...
                        <div className="text-center mb-6 mt-2">
                            <h2 className="text-xl font-bold text-[#1e2a4a]">
                                {viewMode === 'CREATE_MPIN_MOBILE' ? 'Create Admin MPIN' : 'Reset Admin MPIN'}
                            </h2>
                ...
                {(viewMode === 'CREATE_MPIN_SET' || viewMode === 'FORGOT_MPIN_SET') && (
                    <div className="animate-fade-in">
                        ...
                        <button
                            ...
                            className="w-full bg-[#1e2a4a] text-white font-bold py-3.5 rounded-xl shadow-lg transition-all"
                        >
                            {loading ? 'Saving...' : (viewMode === 'CREATE_MPIN_SET' ? 'Save MPIN' : 'Reset MPIN')}
                        </button>
                    </div>
                )}
